...
namespace QLNHANVIENNET
{
    public partial class Form1 : Form
    {
        ...
        string sconn = @"DATA SOURCE = HUYNGUYENPC; INITIAL CATALOG = QL_NHANVIEN_NET; INTEGRATED SECURITY = TRUE; TRUSTSERVERCERTIFICATE = TRUE";
        private void btnlogin_Click(object sender, EventArgs e)
        {
            // Chuỗi kết nối đến cơ sở dữ liệu
            con = new SqlConnection(sconn);
            StreamWriter output = new StreamWriter("D:\\DOWLOAD\\QLNS_11-main\\input.txt");
            try
            {
                con.Open(); // Mở kết nối
